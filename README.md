# Hi, I'm Manuel 👋

### 💻 Computer Science student focused on **Machine Learning, Deep Learning, and Data Science**

---

## 🌐 Connect with me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-juan--candela-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/juan-candela)
[![GitHub](https://img.shields.io/badge/GitHub-MCandelaAI-black?style=for-the-badge&logo=github)](https://github.com/MCandelaAI)

---

## ⚙️ Tech Stack

### 🧠 Machine Learning / Deep Learning
[![PyTorch](https://img.shields.io/badge/PyTorch-red?style=for-the-badge&logo=pytorch)](https://pytorch.org/)
[![Scikit-learn](https://img.shields.io/badge/Scikit--learn-orange?style=for-the-badge&logo=scikitlearn)](https://scikit-learn.org/)

### 📊 Data & Analysis
[![NumPy](https://img.shields.io/badge/NumPy-blue?style=for-the-badge&logo=numpy)](https://numpy.org/)
[![Pandas](https://img.shields.io/badge/Pandas-black?style=for-the-badge&logo=pandas)](https://pandas.pydata.org/)

### 📈 Visualization
[![Matplotlib](https://img.shields.io/badge/Matplotlib-blue?style=for-the-badge)](https://matplotlib.org/)
[![Seaborn](https://img.shields.io/badge/Seaborn-lightblue?style=for-the-badge)](https://seaborn.pydata.org/)
[![Power BI](https://img.shields.io/badge/PowerBI-yellow?style=for-the-badge&logo=powerbi)](https://powerbi.microsoft.com/)

### 🗄️ Databases
[![MySQL](https://img.shields.io/badge/MySQL-blue?style=for-the-badge&logo=mysql)](https://www.mysql.com/)

### 💻 Languages
[![Python](https://img.shields.io/badge/Python-blue?style=for-the-badge&logo=python)](https://www.python.org/)
[![Java](https://img.shields.io/badge/Java-red?style=for-the-badge&logo=java)](https://www.java.com/)
[![SQL](https://img.shields.io/badge/SQL-grey?style=for-the-badge)](https://en.wikipedia.org/wiki/SQL)

---

## 📌 About Me

- 🧪 Focused on **Deep Learning** and **Predictive Modeling**  
- 🛠️ Strong emphasis on **clean, reproducible code**  
- 📊 Experience building **end-to-end ML pipelines**  
- ⚙️ Working with **PyTorch, Scikit-learn, SQL, Power BI**  

---

## 🧩 Featured Projects

### 🧠 Deep Learning
- CNNs & RNNs → Image classification and sequence modeling  
- Autoencoders → Representation learning  
- Reinforcement Learning → Q-Learning, Multi-Armed Bandits  

### 📊 Machine Learning
- Supervised Learning → Regression & Classification pipelines  
- Unsupervised Learning → Clustering & PCA  
- Data Analysis → Full workflow from preprocessing to evaluation  

---

## ⚡ Workflow

> `main` → clean, portfolio-ready projects  
> `notebooks` → experimentation, research, and EDA  

---
